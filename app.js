...
function saveState(){localStorage.setItem(DATA_KEY,JSON.stringify(state));scheduleSync();}
...
function showLiveRoom(sessionId){
  ...
  modal('Live Classroom',`<div class="live-room">
    ...
    <div class="notice" style="margin:10px 15px"><strong>Device controls</strong><span>Camera and microphone permissions are requested when the room opens. If access is denied, the controls remain usable in demo mode.</span></div>
...
document.addEventListener('click',e=>{
  ...
  const demo=e.target.closest('[data-role]');if(demo){state.currentRole=demo.dataset.role;state.currentUserId=state.users.find(u=>u.role===state.currentRole)?.id||state.users[0].id;qsa('.demo-card').forEach(x=>x.classList.toggle('active',x===demo));return}
  ...
  if(a==='logout'){stopLiveTimer();api('/auth/logout',{method:'POST'}).catch(()=>{}).finally(()=>{localStorage.removeItem(TOKEN_KEY);qs('#app').classList.add('hidden');qs('#login-screen').classList.remove('hidden');closeModal();toast('You have been signed out.');});return}
...
qs('#login-form').addEventListener('submit',async e=>{e.preventDefault();const selectedDemo=document.querySelector('.demo-card.active')?.dataset.role;const email=qs('#login-email').value.trim();const password=qs('#login-password').value;const demoEmails={admin:'owner@gmtutoring.co.za',tutor:'thandi@gmtutoring.co.za',learner:'lerato@student.co.za'};const role=demoEmails[selectedDemo]?.toLowerCase()===email.toLowerCase()?selectedDemo:undefined;try{const result=await api('/auth/login',{method:'POST',body:JSON.stringify({email,password,role})});localStorage.setItem(TOKEN_KEY,result.token);state=Object.assign(clone(seed),result.state);state.currentRole=result.user.role;state.currentUserId=result.user.id;saveState();qs('#login-screen').classList.add('hidden');qs('#app').classList.remove('hidden');route='dashboard';backendOnline=true;updateBackendBadge();render();toast(`Signed in as ${roleNames[result.user.role]}.`);}catch(err){toast(err.message,'error')}});
qsa('.demo-card').forEach(x=>x.addEventListener('click',()=>{qsa('.demo-card').forEach(y=>y.classList.remove('active'));x.classList.add('active');const email={admin:'owner@gmtutoring.co.za',tutor:'thandi@gmtutoring.co.za',learner:'lerato@student.co.za'}[x.dataset.role];qs('#login-email').value=email;qs('#login-password').value='Welcome123!';}));
qsa('.demo-card')[0]?.click();
